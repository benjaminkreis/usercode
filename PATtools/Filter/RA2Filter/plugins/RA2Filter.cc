...
class RA2Filter : public edm::EDFilter {
   public:
      explicit RA2Filter(const edm::ParameterSet&);
      ~RA2Filter();
...
RA2Filter::RA2Filter(const edm::ParameterSet& iConfig):
...
bool
RA2Filter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  // using namespace edm;
  nEvents_++;
