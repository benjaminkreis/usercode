...
TString varname = "mWH";
int nbins = 30;
double min = 0; double max = 1800;
  

void loadSamples(TString inputPath = ""){
...
void createRooHistPdfs(TCut cuts){
  initStyle();
  gROOT->SetStyle("CMS");
  ...
  /////////////////////////////////////////
  TH1D* hTotalBackground = new TH1D("hTotalBackground", "hTotalBackground", nbins, min, max);
  ...
  for(unsigned int i=0; i < backgroundVector.size(); i++){
    ...
    TH1D hTemp(thisName, thisName, nbins, min, max);
    hTemp.Sumw2();
  ...
  //////////////////////////////////
  TH1D* hSignal0 = new TH1D("hSignal0", "hSignal0", nbins, min, max);
  hSignal0->Sumw2();
  ...
  TH1D* hSignal1 = new TH1D("hSignal1", "hSignal1", nbins, min, max);
  hSignal1->Sumw2();
  ...
  /////////////////////////////////////////////////////////////////////////////
  RooRealVar mWH(varname, varname, min, max);
...
void doHypothesisTest(){
  ...
  cout << nBkg_ext << " " << nSig0_ext << " " << nSig1_ext << endl;

  //Plot pdfs
  TCanvas *c_pdfs = new TCanvas("c_pdfs", "c_pdfs", 640, 480);
  c_pdfs->cd();
  gPad->SetRightMargin(0.08);
  RooRealVar x(varname, varname, min, max);
  ...
  TH1D* hTestStatistic_data0 = new TH1D("hTestStatistic_data0", "hTestSTatistic_data0", 30, -30, 30);
  TH1D* hTestStatistic_data1 = new TH1D("hTestStatistic_data1", "hTestSTatistic_data1", 30, -30, 30);
  ...
  for(int i=0; i<nToys; i++){
    ...
    double testStatistic_data0 = 2.*(r_data0_model0->minNll()-r_data0_model1->minNll()); //FIXME sign?
    ...
    double testStatistic_data1 = 2.*(r_data1_model0->minNll()-r_data1_model1->minNll()); //FIXME sign?
